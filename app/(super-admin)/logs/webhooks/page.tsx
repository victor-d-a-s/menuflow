...
export default async function WebhooksPage() {
  // Configuração futura do Supabase para buscar os logs
  // const supabase = createAdminClient(process.env.NEXT_PUBLIC_SUPABASE_URL!, process.env.SUPABASE_SERVICE_ROLE_KEY!);
  // const { data: webhooks } = await supabase.from('webhook_logs').select('*');
      ...
      <div className="border rounded-lg bg-card shadow-sm">
        <div className="p-8 flex flex-col items-center justify-center text-center space-y-3 min-h-[400px]">
          ...
          <p className="text-muted-foreground max-w-sm">
            Os registros de comunicação do MenuFlow com sistemas externos, como gateways de pagamento, aparecerão aqui.
          </p>
        </div>
      </div>
    </div>
